...
describe("Purchase Endpoint (Protected)", () => {
  ...
  it("should calculate total price applying the discount percentage", async () => {
    // 1. Create a Sweet with 20% Discount
    const sweet = await Sweet.create({
      name: "Discounted Barfi",
      category: "Milk",
      price: 100,
      quantity: 20,
      images: 'mock_url', 
    ...
    const res = await request(app)
      .post(`/api/v1/sweets/${sweet._id}/purchase`)
      .set("Authorization", `Bearer ${userToken}`)
      .send({ quantity: 2 });

...
const mockUploadImage = jest.fn(() =>
  Promise.resolve(
    "https://res.cloudinary.com/demo/image/upload/v1/mock-sweet.jpg"
  )
);
...
describe("Sweets Endpoints (Protected)", () => {
  ...
  describe(`POST ${ENDPOINT}`, () => {
    ...
    it("should create a new sweet with image URL when Admin uploads file", async () => {
      ...
      expect(res.body.imageUrl).toEqual(
        "https://res.cloudinary.com/demo/image/upload/v1/mock-sweet.jpg"
      );
    ...
    it("should return 500 if Cloudinary upload fails", async () => {
      ...
      mockUploadImage.mockRejectedValueOnce(
        new Error("Cloudinary Service Down")
      );
    ...
    it("should allow admin to create a sweet with a discount percentage", async () => {
      const res = await request(app)
        .post("/api/v1/sweets")
        .set("Authorization", `Bearer ${adminToken}`)
        .field("name", "Discounted Laddu")
        .field("price", 100)
        .field("category", "Ghee")
        .field("quantity", 50)
        .field("discount", 20) // 20% Discount
        .attach("image", Buffer.from("fakeimage"), "laddu.png");

      expect(res.statusCode).toEqual(201);
      expect(res.body.discount).toEqual(20);
    });
    // VALIDATION TEST
    it('should default discount to 0 if not provided', async () => {
      const res = await request(app)
        .post('/api/v1/sweets')
        .set('Authorization', `Bearer ${adminToken}`)
        .field('name', 'Regular Laddu')
        .field('price', 100)
        .field('category', 'Ghee')
        .field('quantity', 50)
        .attach('image', Buffer.from('fakeimage'), 'laddu.png');
